...
module.exports = function(RED)
{
	...
	function HueLight(config)
	{
		...
		this.applyMethods = {
			getLightId: (msgTopic) => {
				const lightId = (!config.lightid && typeof msgTopic != 'undefined' && bridge.validResourceID.test(msgTopic) === true) ? msgTopic : config.lightid; 
				if (!lightId) { scope.error(RED._("hue-light.node.error-no-id")); }
				return lightId;
			},
			getCurrentState: (lightId) => {
				const state = bridge.get("light", lightId, { colornames: config.colornamer ? true : false });
				if (!state) { scope.error("The light in not yet available. Please wait until HueMagic has established a connection with the bridge or check whether the resource ID in the configuration is valid.."); }
				return state;
			},
			duckTypeMessage: (msg) => {
				if (msg.payload === undefined) { return false; }
				if (msg.payload.status !== undefined || msg.__user_inject_props__ == "status") return MSG_TYPES.STATUS;
				if (msg.payload.status !== undefined) return MSG_TYPES.STATUS;
			}
		};
		...
		this.applyCommands = async function(msg, send = null, done = null)
		{
			...
			// GET THE LIGHT ID FROM THE CONFIGURED NODE (if set) OR THE MSG.TOPIC (if node is a generic light)
			const tempLightID = this.applyMethods.getLightId(msg.topic);
			if(!tempLightID) return false;

			// GET THE CURRENT STATE OR RETURN FALSE IF BRIDGE ISN'T READY
			let currentState = this.applyMethods.getCurrentState(tempLightID);
			if (!currentState) return false;

			const msgType = this.applyMethods.duckTypeMessage(msg);
			// IF STATUS IS REQUESTED, SEND THE REQUEST AND BAIL EARLY
			if (msgType == MSG_TYPES.STATUS) {
				if (scope.lastCommand !== null) { currentState.command = scope.lastCommand; }
				scope.send(currentState);
				scope.lastCommand = null;
				if (done) done();
				return true;
